# Compress an Image using saving Options (CompressUsingOptions)

When saving raster images in JPG format the default JPG compression and conversion options are applied. ImageGear allows you to set these options manually.

This sample demonstrates how to set the JPEG compression type to Lossy and the JPEG compression quality to 30. The sample code also sets the pixel converter for manipulating bit depths to ANY and the color space to None, which are the default settings for these parameters.
...
2. Run `dotnet build CompressUsingOptions.sln`. By default this will build the Debug Solution Configuration.
