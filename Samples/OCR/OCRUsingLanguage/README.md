# OCR Using a Specific Language (OCRUsingLanguage)

In order to best OCR the contents of a page, ImageGear can set the OCR language to improve recognition accuracy.

More than one language can be enabled, but only certain languages can be enabled together. Check documentation for details.
