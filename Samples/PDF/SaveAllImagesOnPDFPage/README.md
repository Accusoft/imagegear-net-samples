# Save All Images On a PDF Page (SaveAllImagesOnPDFPage)

When a PDF page contains embedded raster images, ImageGear and extract those raster images and then use and save them like any other raster image.

After a PDF page is loaded, the objects on the page can be traversed. In this case, raster image objects on the PDF page are converted into ImageGear raster images using the ToImGearPage function.
...
- Build the sample using `dotnet build SaveAllImagesOnPDFPage.dotnet.sln`. By default this will build the Debug Solution Configuration.
...
To run this sample under Linux, run the sample from "bin/x64/Debug/net6.0/" using `./SaveAllImagesOnPDFPage.dotnet`.
