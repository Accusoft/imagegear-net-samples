# Verify a PDF Signature (VerifyPDFSignature)

When a PDF is digitally signed, ImageGear can verify that the digital signature is valid.

After providing certificates to ImageGear, all the signatures in a PDF document can be verified at once with the VerifySignatures function. You can also use the Signatures property of the PDF document to verify each signature separately and get information about each signature.
