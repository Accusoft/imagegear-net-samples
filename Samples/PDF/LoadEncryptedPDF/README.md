# Load Encrypted PDF (LoadEncryptedPDF)

When using ImageGear with encrypted PDF files, ImageGear can decrypt and load a PDF.

This sample demonstrates how to set an authorization callback procedure, which is required to decrypt and load a secure PDF. Additionally, it demonstrates how to remove encryption from a PDF once it has been loaded.
...
2. Run `dotnet build LoadEncryptedPDF.sln`. By default this will build the Debug Solution Configuration.
