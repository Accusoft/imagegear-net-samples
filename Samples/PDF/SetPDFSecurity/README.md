# Set PDF Security (SetPDFSecurity)

When the content of a PDF needs to be protected, ImageGear can set the PDF security.
...
To build this sample under Linux, first install the Microsoft .NET SDK for your Linux distribution. After that, run `dotnet build SetPDFSecurity.dotnet.sln`. By default this will build the Debug Solution Configuration.
...
To run this sample under Linux, run the sample from "bin/x64/Debug/net6.0/" using `./SetPDFSecurity.dotnet`.
