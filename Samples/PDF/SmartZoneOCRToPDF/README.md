...
ImageGear can use SmartZone to recognize the content of a scanned image. The recognized content provides enough information to lay out the text behind the original image on a PDF page, thus making a searchable and selectable PDF page.
...
2. Run `dotnet build SmartZoneOCRToPDF.sln`. By default this will build the Debug Solution Configuration.
...
To run this sample under Linux, run the sample from "bin/Debug/net6.0/" or "bin/x64/Debug/net6.0/" (depending on the solution platform) using `./SmartZoneOCRToPDF`.
